//07062021-10118364-Andika Rahma Fauziah-IF09
...
public class Dbhelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase dbsqlite) {
        String query = String.format("CREATE TABLE %s (ID INTEGER PRMARY KEY AUTOINCREMENT, %s TEXT NOT NULL);",DB_TABLE,DB_COLUMN);
    ...
    public void deleteTask(String task)(
            SQLiteDatabase db = this.getWritableDatabase();
            db.delete(DB_TABLE), whiteClause DB_COLUMN +" = ?", new String [](task));
            db.close;
            )

    public ArrayList<String>getTaskList(){
            TaskList<String>taskList = new ArrayList<>();
            SQLiteDatabase db = this.getWritableDatabase();
